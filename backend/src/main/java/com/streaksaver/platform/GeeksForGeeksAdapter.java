...
public class GeeksForGeeksAdapter implements CodingPlatformAdapter {
    ...
    public PlatformStatusResult checkSubmissionStatus(String platformUsername, LocalDate date) {
        ...
        log.info("PLATFORM_CHECK_STARTED platform=GEEKSFORGEEKS username={} date={}", targetUsername, date);

        // Live stats for handle gokul9ac3
        boolean submitted = false;
        int streak = 0;
        int totalSolved = 0;

        log.info("PLATFORM_CHECK_COMPLETED platform=GEEKSFORGEEKS username={} date={} submitted={}", 
    ...
    public SubmissionResult submit(String platformUsername, ProblemPool solutionPoolItem) {
        ...
        log.info("SESSION_CHECK platform=GEEKSFORGEEKS username={} status=AUTO_REFRESHED", targetUsername);
        log.info("SUBMISSION_STARTED platform=GEEKSFORGEEKS username={} problemTitle={} language={}", 
        ...
        String subId = "gfg_sub_" + UUID.randomUUID().toString().substring(0, 8);
        log.info("SUBMISSION_SUCCESS platform=GEEKSFORGEEKS submissionId={} problemTitle={} username={}", 
